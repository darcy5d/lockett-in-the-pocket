...
class AFLDataLoader:
    ...
    def __init__(self, data_dir=None):
        ...
        Args:
            data_dir (str, optional): Path to the data directory. 
                                     If None, will use "afl_data" in the project root.
        """
        if data_dir is None:
            # Get the project root directory (two levels up from this script)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = os.path.dirname(script_dir)
            data_dir = os.path.join(project_root, "afl_data")
        ...
        if not os.path.exists(self.data_dir):
            print(f"Data directory {self.data_dir} does not exist. Please run fetch_afl_data.py from the datafetch directory first.")
    ...
    def load_matches(self, year=None):
        ...
        if not os.path.exists(self.match_dir):
            print(f"Match directory {self.match_dir} does not exist. Please run fetch_afl_data.py from the datafetch directory first.")
            return None
    ...
    def load_player_data(self, player_name=None, data_type="performance"):
        ...
        if not os.path.exists(self.player_dir):
            print(f"Player directory {self.player_dir} does not exist. Please run fetch_afl_data.py from the datafetch directory first.")
            return None
    ...
    def load_odds_data(self, year=None):
        ...
        if not os.path.exists(self.odds_dir):
            print(f"Odds directory {self.odds_dir} does not exist. Please run fetch_afl_data.py from the datafetch directory first.")
            return None
...
def main():
    ...
    if not os.path.exists(loader.data_dir):
        print(f"Data directory '{loader.data_dir}' not found. Please run fetch_afl_data.py from the datafetch directory first.")
        return
    ...
    print("\nExample Usage:")
    print("from datafetch.load_afl_data import AFLDataLoader")
