#!/usr/bin/env python3
"""
Fetch the 2026 UK Championship fixture from League Unlimited.

Source: https://leagueunlimited.com/competition/show/championship/2026/draw/full
Output: matches_championship-uk_2026.csv (per competition_config)

Usage:
    python datafetch/fetch_2026_championship_fixture.py
"""
...
OUTPUT_PATH = _PROJECT_ROOT / "nrl_data" / "data" / "matches" / "matches_championship-uk_2026.csv"
LEAGUE_UNLIMITED_BASE = "https://leagueunlimited.com/competition/show/championship/2026/draw/full"
YEAR = 2026
...
def _strip_score(team_raw: str) -> str:
    """Strip (score) from team name e.g. 'Salford RLFC (0)' -> 'Salford RLFC'."""
    return re.sub(r"\s*\(\d+\)\s*$", "", team_raw).strip()


def _parse_match_link(text: str, href: str) -> dict | None:
    """
    Parse 'London Broncos v Salford RLFC Sun 22 Mar 3:00PM at The Cherry Red Records Stadium, Wimbledon'
    or 'Salford RLFC (0) v Oldham RLFC (44) Fri 16 Jan 8:00PM at CorpAcq Stadium (Crowd: 5000)'
    """
    ...
    if len(parts) != 2 or " at " not in parts[1]:
        return None
    team1_raw = _strip_score(parts[0].strip())
    rest = parts[1].strip()
    team2_datetime, venue_part = rest.split(" at ", 1)
    team2_raw, date_str = _extract_team2_and_datetime(team2_datetime)
    team2_raw = _strip_score(team2_raw)
    venue_raw = venue_part.split("(")[0].strip().rstrip(",").strip()
